...
export default function ProtocolDetailPage() {
    ...
    const relatedCreditNote = protocol ? creditNotesMap.get(protocol.creditNoteId) : null;
            ...
            <div className="space-y-6">
                <Card>
                    ...
                    <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <InfoItem label="Valor Solicitado" value={formatCurrency(protocol.value)} />
                        ...
                        />
                        <InfoItem label="Criado por" value={creatorUser?.email || protocol.userId} />
                    </CardContent>
                </Card>
                
                 <Card>
                    <CardHeader>
                        <CardTitle>Vinculação Financeira</CardTitle>
                    </CardHeader>
                    <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <InfoItem 
                            label="Nota de Crédito" 
                            value={relatedCreditNote ? <Link href={`/dashboard/credit-notes/${protocol.creditNoteId}`} className="text-primary hover:underline">{relatedCreditNote.ncNumber}</Link> : 'N/A'} 
                        />
                        <InfoItem label="UASG da NC" value={relatedCreditNote?.uasg} />
                        <InfoItem label="PI da NC" value={relatedCreditNote?.pi} />
                        <InfoItem label="ND da NC" value={relatedCreditNote?.nd} />
                    </CardContent>
                </Card>
                 ...
                 {protocol.neNumber && (
                     <Card>
                        ...
                        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <InfoItem label="Nº do Empenho" value={protocol.neNumber} />
                            <InfoItem label="Data do Empenho" value={formattedDate(protocol.neDate)} />
                        </CardContent>
                    </Card>
                 )}
                        ...
                        <div key={index} className="flex items-center gap-4">
                            ...
                            <div className="grid gap-1">
                                <p className="font-semibold">
                                    {item.status}
                                    <span className="font-normal text-muted-foreground ml-2">por {item.user?.email || item.user?.uid || 'Sistema'}</span>
                                </p>
                                        ...
                                        <div key={index} className="flex items-start gap-4 p-3 border-b last:border-b-0">
                                            <div className="grid gap-1.5 w-full">
                                                <div className="flex items-center justify-between">
                                                    <p className="font-semibold">{obsUser?.email || obs.userId}</p>
