...
export default function CreditNoteDetailPage() {
    ...
    const protocolsQuery = useMemoFirebase(() => {
        if (!firestore) return null;
        return query(collection(firestore, "protocols"), where("creditSources", "array-contains", { creditNoteId: creditNoteId }));
    }, [firestore, creditNoteId]);
    ...
    const isLoading = isLoadingNote || isLoadingOms || isLoadingProtocols || isLoadingUsers;
                ...
                <Card>
                    ...
                    <CardContent>
                         <Table>
                            ...
                            <TableBody>
                                 {!isLoadingProtocols && (!protocols || protocols.length === 0) ? (
                                    <TableRow>
                                        <TableCell colSpan={8} className="h-24 text-center">Nenhum protocolo vinculado a esta NC.</TableCell>
                                    </TableRow>
                                ) : (
                                    protocols?.map(protocol => {
                                         ...
                                         return (
                                            <TableRow key={protocol.id}>
                                                <TableCell className="font-medium">{protocol.controlCode}</TableCell>
                                                ...
                                                <TableCell>{protocol.commitments?.[0]?.neNumber || '—'}</TableCell>
                                                <TableCell>{formattedDate(protocol.commitments?.[0]?.neDate)}</TableCell>
                                                <TableCell className="text-right">{formatCurrency(protocol.value)}</TableCell>
                                            </TableRow>
                                        )
                                    })
                                )}
                                {isLoadingProtocols && (
                                    <TableRow><TableCell colSpan={8} className="h-24 text-center">Carregando protocolos...</TableCell></TableRow>
                                )}
                            </TableBody>
                        </Table>
                    </CardContent>
